 ...
 * @LastEditTime: 2023-08-23 22:05:48
...
import { fetchDoPlot, fetchDoPlotTest, fetchRedrawPlot, fetchOpenPictures } from '@/api';
import './index.less';
...
function DrawingProduction() {
    ...
    const getData = async () => {
        ...
        try {
            ...
            const imgListWithRedrawKey = res.img_list.map((item) => ({
                ...item,
                redrawKey: 0, // 初始化重绘键
                isLoading: false, // 初始化加载状态
            }));
            setData(imgListWithRedrawKey);
        } catch (error) {
            message.error(error);
            setLoading(false);
        }
    ...
    const openPictures = async () => {
        const res = await fetchOpenPictures();
    };

    const redrawPlot = async (itemToRedraw) => {
        // 设置特定项的加载状态为 true
        setData((prevData) =>
            prevData.map((item) => (item.img_name === itemToRedraw.img_name ? { ...item, isLoading: true } : item))
        );

        console.log({ ...fromValue, prompt: `((best quality)), ((masterpiece)), (detailed),${itemToRedraw.future}` });
        const res = await fetchRedrawPlot({
            image: itemToRedraw.img_name,
            paint_set: { ...fromValue, prompt: `((best quality)), ((masterpiece)), (detailed),${itemToRedraw.future}` },
        });

        // 重绘完成后，更新特定项的重绘键并设置加载状态为 false
        setData((prevData) =>
            prevData.map((item) =>
                item.img_name === itemToRedraw.img_name
                    ? { ...item, redrawKey: item.redrawKey + 1, isLoading: false }
                    : item
            )
        );
    };
    ...
    //     console.log({ ...fromValue, prompt: `((best quality)), ((masterpiece)), (detailed),${item.future}` });
    //     const res = await fetchRedrawPlot({
    //         image: item.img_name,
    //         paint_set: { ...fromValue, prompt: `((best quality)), ((masterpiece)), (detailed),${item.future}` },
    //     });
        ...
        <Spin spinning={loading}>
            <div className="drawing-production-btn">
                ...
                <Space wrap>
                    ...
                    <Button onClick={openPictures}>打开图片文件夹</Button>
            ...
            <div className="drawing-image">
                <Space wrap>
                    {data.map((item) => {
                        return (
                            <div className="gutter-row" span={6} key={item.img_name}>
                                <Spin spinning={item.isLoading}>
                                    <Card
                                        ...
                                        cover={
                                            <Image
                                                width={350}
                                                src={`http://localhost:12002/to_video/images/${item.img_name}?redraw=${item.redrawKey}`}
                                            />
                                        }
                                        actions={[
                                    ...
                                    >
                                        {editingItem === item ? (
                                            <>
                                                <div className="drawing-texts">{item.texts}</div>
